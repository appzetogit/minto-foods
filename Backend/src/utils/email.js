...
export async function sendAdminResetOtpEmail(to, otp) {
    ...
    const subject = 'Your password reset code – Minto Foods Admin';
    const html = `
<!DOCTYPE html>
<html>
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 480px; margin: 0 auto; padding: 20px;">
  ...
  <p style="color: #999; font-size: 12px;">Minto Foods Admin</p>
</body>
</html>`;
    ...
    try {
        await trans.sendMail({
            from: typeof from === 'string' && from.includes('<') ? from : `Minto Foods <${from}>`,
            to,
            subject,
            text,
            html
